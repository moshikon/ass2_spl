...
public abstract class MicroService implements Runnable {
	...
	private Hashtable<Class<?> ,Callback<?>> Callbacks_ = new Hashtable<Class<?> ,Callback<?>>();	
	private Hashtable<Request<?> ,Callback<?>> Requests_ = new Hashtable<Request<?> ,Callback<?>>();	
	...
	public final <R extends Request> void subscribeRequest(Class<R> type, Callback<R> callback) {       
    	Callbacks_.put(type,callback);	
	...
	public final void run() {
        ...
        while (!terminated) { 
        	...
        	try {
				...
				e.printStackTrace();
	            throw new IllegalArgumentException("problem with microservice");
			}
			if (msg.getClass() == RequestCompleted.class){
				RequestCompleted temp = (RequestCompleted) msg;
    			Callback curr = Requests_.get(temp.getCompletedRequest());
    			curr.call(temp.getResult());
    		}else{
    			Callback curr = Callbacks_.get(msg.getClass());
    			curr.call(msg);
    			}
    		}
